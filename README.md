...
- Lidarr tracked artists and album completion statistics are cached by a five-minute library scan. Artist pages suppress duplicate add controls, while discographies show whether each release group is fully available, incomplete and searchable again, or ready to request; the job can also be run manually from **Settings → Jobs & Cache**.
